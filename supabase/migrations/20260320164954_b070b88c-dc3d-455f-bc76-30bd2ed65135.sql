...
BEGIN
  ...
  IF p_feature_key IN ('card_creation', 'create_card', 'uzum_card_creation', 'wb_card_creation', 'yandex_card_creation') 
    ...
    IF COALESCE(v_usage, 0) < v_plan.free_card_creation_monthly THEN
      RETURN jsonb_build_object('allowed', true, 'price', 0, 'tier', COALESCE(v_sub.plan_slug, 'free'), 
  ...
  RETURN jsonb_build_object('allowed', true, 'price', v_price, 'tier', v_tier, 'discount', v_discount);
END;
$function$;