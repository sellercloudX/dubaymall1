-- Update check_feature_access to remove premium_only check (replaced by plan-based inclusion)
...
BEGIN
  ...
  IF p_feature_key IN ('clone_card', 'card_cloning', 'clone-to-yandex', 'clone-to-wildberries', 'clone-to-uzum') AND v_plan.free_cloning_monthly > 0 THEN
    SELECT COALESCE(usage_count, 0) INTO v_usage 
    FROM elegant_usage 
    WHERE user_id = p_user_id AND feature_key = p_feature_key 
      AND usage_month = (date_trunc('month', now()))::date;
